"""Research analysis using supplied measurements and portable output paths."""
...
def main():
    ...
    mape_test = (
        np.mean(
            np.abs(
                (actual_concentration_test - predicted_concentration_test)
                / actual_concentration_test
            )
        )
        * 100
    )
    ...
    print(f"R^2 (Training Data): {r2_train:.4f}")
    ...
    print(f"R^2 (Testing Data): {r2_test:.4f}")
